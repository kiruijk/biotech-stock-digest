...
async function getCompanyInfo(symbol) {
  const summary = await withRetry(() => yahooFinance.quoteSummary(symbol, { modules: ['assetProfile', 'insiderTransactions'] }));
...
async function buildStockData(symbol, previous) {
  ...
  const returns = await getReturns(symbol, quote, quote.regularMarketPrice);
  ...
  try {
    ...
    companyInfo = { company: previous?.company || {}, insiders: previous?.insiders || [] };
  }
...
async function updateAll() {
  ...
  const homepageData = Object.fromEntries(Object.entries(stockData).map(([symbol, d]) => {
    const { company, insiders, news, ...rest } = d;
